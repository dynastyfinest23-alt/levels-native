...
- **Tables:** `users`, `ascension_loops`, `phase1_assessments` (unique per loop), `phase2_dashboard_views`, `phase3_origin_drills`, `phase4_track_sessions`, `embodiment_daily_logs`, `phase5_reassessments`, plus v1.1 additions `user_calibration`, `energy_guides`. Also `waitlist` — owned by `marketing-site/`, not the app: anon INSERT via `join_waitlist(p_email)`, no client role can read it. Created out-of-band in the SQL editor and only brought under migration control 2026-07-28 (`20260728203732_declare_waitlist_schema.sql`).
...
- **Security:** RLS on all user tables (SELECT/INSERT/UPDATE scoped to `auth.uid() = user_id`). All SECURITY DEFINER functions hardened with `SET search_path = public, pg_temp` — since 2026-07-28 the five SECURITY INVOKER scoring functions (`answer_to_raw_score`, `apply_downward_anchor_weight`, `score_to_zone`, `assign_phase4_track`, `compute_phase5_classification`) are pinned too, so a clean `get_advisors` run means zero `function_search_path_mutable` warnings. Preserve both properties in any new function. **Trigger functions get no EXECUTE grant:** `handle_new_user` and `seed_calibration_from_assessment` are reachable only from their triggers, never over `/rest/v1/rpc/` (revoked 2026-07-28, `20260728203733`); a trigger fires regardless of the caller's EXECUTE privilege.
...
- (Resolved 2026-08-03) Test-account cleanup: 29 accounts deleted, `auth.users` down to 4. The long-standing "needs the service-role/admin API this client doesn't have" framing was true only of the Flutter client: the Supabase MCP connects as `postgres` (`rolbypassrls`), and `public.users.id → auth.users.id` is ON DELETE CASCADE, so a single `DELETE FROM auth.users` clears auth rows plus every phase row with zero orphans. Use that path for future cleanups. Preserved: the three real accounts plus one `m52-ui-verify` account created 2026-08-03 06:31 UTC whose origin is unidentified (see `ACTION-FOR-NOAH.md`).
